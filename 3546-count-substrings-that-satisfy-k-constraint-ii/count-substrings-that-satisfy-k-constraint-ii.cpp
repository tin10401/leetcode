...
class SGT
{
    ...
    void update(iterator, int start, int end, ll val)
    {
        pushDown;
    ...
    ll queries(iterator, int start, int end)
    {
        pushDown;
...
public:
    vector<long long> countKConstraintSubstrings(string s, int k, vector<vector<int>>& queries) {
        ...
        for(int i = 0; i < m; i++)
        {
            Q[queries[i][1]].pb({queries[i][0], i});
        }
        ll ans = 0;
        for(int left = 0, right = 0, one = 0, zero = 0; right < n; right++)
        {
            if(s[right] == '0') zero++;
            else one++;
            while(one > k && zero > k)
            ...
            ans += right - left + 1;
            for(auto& [l, index] : Q[right])
            {
                res[index] += ans - (l == 0 ? 0 : root.queries(0, l - 1));
            }
        }
        return res;
    }
