...
public:
    string numberToWords(int num) {
        ...
        {80, "Eighty"},
        {90, "Ninety" }};
        auto getString = [&](int n) -> string
        {
            if(n == 0) return "";
            string res;
            int t = n / 100;
            if(t) res = mp[t] + " Hundred";
            int h = n % 100;
            if(h == 0) return res;
            if(!res.empty()) res += " ";
            if(h <= 20) res += mp[h];
            ...
            {
                int last_2 = h - h % 10, last_1 = h % 10;
                res += mp[last_2];
                if(last_1) res += " " + mp[last_1];
            }
            return res;
        };
        vs arr = {{""}, {" Thousand"}, {" Million"}, {" Billion"}};
        vs list;
        int i = 0;
        while(num)
        {
            string s = getString(num % 1000);
            if(!s.empty())
            {
                list.pb(s + arr[i] + " ");
            } 
