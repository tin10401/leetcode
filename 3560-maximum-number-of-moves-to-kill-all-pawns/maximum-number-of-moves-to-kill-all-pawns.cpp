...
public:
    int maxMoves(int kx, int ky, vector<vector<int>>& positions) {
        ...
        auto compute = [&](int x, int y) -> vi {
            vi dis(M * M, -1);
            dis[x * M + y] = 0;
            queue<ar(3)> q;
            q.push({x, y, 0});
            while(!q.empty()) {
                auto [r, c, cost] = q.front(); q.pop();
                for(auto& it : dirs) {
                    int row = r + it[0], col = c + it[1];
                    int nx = row * M + col;
                    if(row >= 0 && col >= 0 && row < M && col < M && dis[nx] == -1) {
                        dis[nx] = cost + 1;
        ...
        for(auto& it : positions) {
            DIS.pb(compute(it[0], it[1]));
        }
        int target = (1 << n) - 1;
        auto dfs = [&](auto& dfs, int mask, int id, bool turn, vt<vvi>& dp) {
            if(mask == target) return 0;
            int& res = dp[id][turn][mask];
            if(res != -1) return res;
            res = turn ? -INF : INF;
            for(int i = 0; i < n; i++) {
                ...
                x = x * M + y;
                int ans = DIS[id][x] + dfs(dfs, mask ^ (1 << i), i, !turn, dp);
                res = turn ? max(res, ans) : min(res, ans);
            }
            return res;
        };
        vi curr = compute(kx, ky);
        int res = -INF;
        for(int i = 0; i < n; i++) {
            int x = positions[i][0], y = positions[i][1];
            vt<vvi> dp(n, vvi(2, vi(1 << n, -1)));
            res = max(res, curr[x * M + y] + dfs(dfs, 1 << i, i, false, dp));
        }
        return res;
    }
};